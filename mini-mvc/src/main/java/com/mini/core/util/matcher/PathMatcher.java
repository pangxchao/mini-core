...
public interface PathMatcher {
	 ...
	 * method does not have to be used because direct equality comparisons on the static path Strings will lead to the
	 * same result.
	 ...
	 * dynamically through an actual pattern, that is, it strips off a statically defined leading path from the given
	 * full path, returning only the actually
	 * pattern-matched part of the path.
	 * <p>For example: For "myroot/*.html" as pattern and "myroot/myfile.html"
	 * as full path, this method should return "myfile.html". The detailed determination rules are specified to this
	 * PathMatcher's matching strategy.
	 * <p>A simple implementation may return the given full path as-is in case
	 * of an actual pattern, and the empty String in case of the pattern not containing any dynamic parts (i.e. the
	 * {@code pattern} parameter being a static
	 * path that wouldn't qualify as an actual {@link #isPattern pattern}). A sophisticated implementation will
	 * differentiate between the static parts and the
	 ...
	 * but generally, the returned {@code Comparator} will {@linkplain java.util.List#sort(java.util.Comparator) sort}
	 * a list so that more specific patterns
