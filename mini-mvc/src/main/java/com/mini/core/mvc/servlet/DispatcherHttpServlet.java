package com.mini.core.mvc.servlet;
...
import static com.mini.core.mvc.validation.ValidateUtil.message;
import static com.mini.core.mvc.validation.ValidateUtil.validateParameters;
 ...
 */
@Component
public final class DispatcherHttpServlet extends HttpServlet implements ResponseCode {
    ...
    @Autowired
    public void setApplication(AnnotationConfigApplicationContext application) {
        this.application = application;
    }

    public AnnotationConfigApplicationContext getApplication() {
        return application;
    }
    ...
    private void doService(Action.Method method, HttpServletRequest request, HttpServletResponse response) {
        try {
            ...
            String uri = requireNonNullElse(request.getRequestURI(), "").replaceFirst(contextPath, "");
            ...
            // 根据请求方法获取具体映射,不支持时返回405错误
            if (Stream.of(proxy.getSupportMethod()).noneMatch(m -> m == method)) {
                response.sendError(METHOD_NOT_ALLOWED);
                return;
            }
            // 获取 Ant 类型的URL中的参数信息
            ...
            if (matcher.isPattern((path = proxy.getViewPath()))) {
                uriParam.putAll(matcher.extractUriTemplateVariables(path, uri));
            }
            // 获取数据模型实例并验证是否为空
            ...
            // 获取控制类的实例对象
            Object instance = application.getBean(proxy.getClazz());
            ...
            final ActionInvocation action = new ActionInvocation() {
                ...
                public synchronized final Object[] getParameterValues() {
                    return Stream.of(proxy.getParameterHandlers()).map(param -> {
                        try {
                            ...
                            throw message("{BAD_REQUEST}").send();
                        }
                    }).toArray(Object[]::new);
                }
                ...
                public synchronized final Object invoke() throws Throwable {
                    try {
                        ...
                        final Object[] values = getParameterValues();
                        validateParameters(instance, getMethod(), values);
