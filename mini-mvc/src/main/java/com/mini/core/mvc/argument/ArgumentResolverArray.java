...
public abstract class ArgumentResolverArray extends ArgumentResolverAbstract  {

    protected ArgumentResolverArray(@NotNull Configures configures) {
        ArgumentResolverSupport.init(configures);
    }

    @Override
    public boolean supportParameter(MethodParameter parameter) {
        return ArgumentResolverSupport.getArrayFunc(parameter.getParameterType()) != null;
    }

    @Override
    public final Object getValue(MethodParameter parameter, ActionInvocation invocation) {
        Function<String[], ?> function = ArgumentResolverSupport.getArrayFunc(parameter.getParameterType());
