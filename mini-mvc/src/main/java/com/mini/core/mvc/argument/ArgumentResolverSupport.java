...
public final class ArgumentResolverSupport implements Serializable, EventListener {
	...
	public static void init(@NotNull Configures configures) {
	...
	private static final Function<? super String, ? extends Instant> java_time_instant = text ->
			ofEpochMilli(Long.parseLong(text));
	
	// java.time.LocalDateTime 类型的参数
	private static final Function<? super String, ? extends LocalDateTime> java_time_local_date_time = text -> {
		try {
			DateTimeFormatter format = ofPattern(configures.getDateTimeFormat());
	...
	private static final Function<? super String, ? extends LocalDate> java_time_local_date = text -> {
		try {
			DateTimeFormatter format = ofPattern(configures.getDateTimeFormat());
	...
	private static final Function<? super String, ? extends LocalTime> java_time_local_time = text -> {
		try {
			DateTimeFormatter format = ofPattern(configures.getDateTimeFormat());
	...
	private static final Map<Class<?>, Function<String, Object>> BASIC_MAP = new HashMap<>() {{
		...
		// java.time.Instant 类型的参数
		put(Instant.class, value -> ofNullable(value)
				.filter(v -> !v.isBlank())
				.map(java_time_instant)
				.orElse(null));
		
		// java.time.LocalDateTime 类型的参数
		put(LocalDateTime.class, value -> ofNullable(value)
				.filter(v -> !v.isBlank())
				.map(java_time_local_date_time)
				.orElse(null));
		
		// java.time.LocalDate 类型的参数
		put(LocalDate.class, value -> ofNullable(value)
				.filter(v -> !v.isBlank())
				.map(java_time_local_date)
				.orElse(null));
		
		// java.time.LocalTime 类型的参数
		put(LocalTime.class, value -> ofNullable(value)
				.filter(v -> !v.isBlank())
	...
	private static final Map<Class<?>, Function<String[], Object>> ARRAY_MAP = new HashMap<>() {{
		...
		// java.time.Instant 类型的参数
		put(Instant[].class, values -> Stream.ofNullable(values)
				.flatMap(Stream::of)
				.filter(value -> !value.isBlank())
				.map(java_time_instant)
				.toArray(Instant[]::new));
		...
		// java.time.LocalDateTime[] 类型的参数
		put(LocalDateTime[].class, values -> Stream.ofNullable(values)
				.flatMap(Stream::of)
				.filter(value -> !value.isBlank())
				.map(java_time_local_date_time)
		...
		// java.time.LocalDate[] 类型的参数
		put(LocalDate[].class, values -> Stream.ofNullable(values)
				.flatMap(Stream::of)
				.filter(value -> !value.isBlank())
				.map(java_time_local_date)
		...
		// java.time.LocalTime 类型的参数
		put(LocalTime[].class, values -> Stream.ofNullable(values)
				.flatMap(Stream::of)
				.filter(value -> !value.isBlank())
				.map(java_time_local_time)
