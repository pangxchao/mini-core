            ...
            if (model.getContentLength() > 0) {
                ...
                if (null != request.getHeader("Range")) {
                    ...
                    try { // 从某字节开始 的下载(bytes=27000-)
                        ...
                        contentLength = endPos - startPos + 1; // 只有设置了文件下载大小时， 才支持断点续传
                        // Content-Range: bytes [文件块的开始字节]-[文件的总大小 - 1]/[文件的总大小]
                        response.setHeader("Content-Range", StringUtil.join("bytes ", //
                                startPos, "-", endPos, "/", fileLength));
