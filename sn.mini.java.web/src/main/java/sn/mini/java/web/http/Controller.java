...
public abstract class Controller {
	...
	protected final void doService(HttpServletRequest req, HttpServletResponse response) throws ServletException, IOException {
		try {
	...
	protected final String loginInterceptor(ActionProxy proxy, IModel model, SNHttpServletRequest request, HttpServletResponse response) throws Exception {
		try {
			...
			if (proxy.getAction().login() && user == null) {
				...
				if (StringUtil.isNotBlank(SNInitializer.getLoginUrl())) { return StringUtil.join("r:", SNInitializer.getLoginUrl(), "?uri=",
						StringUtil.urlEncode(StringUtil.join(proxy.getName(), "?", request.getQueryString()), SNInitializer.getEncoding())); }
