		...
		<div class="layui-form-item">
			<div class="layui-input-block mini-input-block">
				<button class="layui-btn layui-btn-sm"
						lay-submit
						lay-filter="searchForm">搜索
				</button>
				<button type="reset"
						class="layui-btn layui-btn-sm layui-btn-primary">清空/重置
				</button>
			</div>
		</div>
	</form>
...
<script type="text/javascript">
	layui.use(['table', 'form', 'page', 'wind'], function () {
		...
		// 该 options 为Ajax相关数据
		page.load('front/user/pages.htm', {});
			...
			layui.wind.template('#insertTemplate', {
				...
				success: function (lay, index) {
					...
					form.on("submit(insertButton)", function (data) {
						layui.ajax('back/user/insert.htm', {
			...
			layui.wind.template('#updateTemplate', {
				...
				success: function (lay, index) {
					...
					form.on("submit(updateButton)", function (data) {
						layui.ajax('back/user/update.htm', {
		...
		var deleteFunction = function (idList) {
			...
			layer.confirm("是否删除选中数据", function (index) {
				layui.ajax('back/user/delete.htm', {
					data: {idList: idList},
					success: function () {
						layer.msg("删除用户成功");
						layer.close(index);
