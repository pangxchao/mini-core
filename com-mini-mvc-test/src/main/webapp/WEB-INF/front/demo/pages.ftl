		...
		<tr>
			...
			<td>
                <#if item.fullHeadUrl?? && item.fullHeadUrl != '' >
					<img src="${item.fullHeadUrl}"
							alt="${item.fullHeadUrl}" />
                </#if>
			</td>
			...
			<td>
				<a class="layui-btn layui-btn-primary layui-btn-xs"
						href="https://www.layui.com/demo/form.html?id=${item.id?c}"
						target="_blank">
					查看
				</a>
				<a class="layui-btn layui-btn-xs"
						href="javascript:window.updateFunction('${item.id?c}')">
					编辑
				</a>
				<a class="layui-btn layui-btn-danger layui-btn-xs"
						href="javascript:window.deleteFunction(['${item.id?c}'])">
					删除
				</a>
			</td>
		</tr>
