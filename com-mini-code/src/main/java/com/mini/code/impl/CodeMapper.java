...
public final class CodeMapper {
	...
	protected static void run(Configure configure, ClassInfo info, String tableName, String prefix, //
		...
		// 生成类信息
		TypeSpec.Builder builder = TypeSpec.classBuilder(info.mapperName).addModifiers(PUBLIC)
				.addSuperinterface(ParameterizedTypeName.get(ClassName.get(IMapper.class), info.beanClass)).addAnnotation(Singleton.class)
				.addAnnotation(AnnotationSpec.builder(Named.class).addMember("value", "$S", firstLowerCase(info.mapperName)).build())
				.addJavadoc("$L.java \n", info.mapperName).addJavadoc("@author xchao \n");
	...
	private static MethodSpec.Builder get(ClassInfo info, List<Util.FieldInfo> fieldList) {
		MethodSpec.Builder method = MethodSpec.methodBuilder("get").addAnnotation(Override.class).addModifiers(PUBLIC)
				.returns(info.beanClass).addParameter(ResultSet.class, "rs").addParameter(int.class, "number")
	...
	private static TypeSpec.Builder builder(ClassInfo info, List<Util.FieldInfo> fieldList) {
		MethodSpec.Builder method = MethodSpec.constructorBuilder().addModifiers(PUBLIC);
		for (Util.FieldInfo fieldInfo : fieldList) {
			String db_name = fieldInfo.getFieldName().toUpperCase();
		...
		return TypeSpec.classBuilder(format("%sBuilder", info.beanName)).addModifiers(PUBLIC, STATIC).superclass(SQLBuilder.class)
				.addJavadoc("$N.java \n", format("%sBuilder", info.beanName)).addJavadoc("@author xchao \n").addMethod(method.build());
	}
