...
public class CodeController {
	...
	protected static void run(Configure configure, ClassInfo info, String tableName, String prefix, //
		...
		// 生成类信息
		TypeSpec.Builder builder = TypeSpec.classBuilder(info.controllerName).addModifiers(PUBLIC).addSuperinterface(IStatus.class)
				.addAnnotation(Singleton.class)
				.addAnnotation(AnnotationSpec.builder(Controller.class).addMember("path", "$S", "back/" + firstLowerCase(info.beanName))
						.addMember("url", "$S", "back/" + firstLowerCase(info.beanName)).build())
				.addJavadoc("$L.java \n", info.controllerName).addJavadoc("@author xchao \n");

		// Service 依赖注入属性定义
		builder.addField(FieldSpec.builder(info.daoClass, firstLowerCase(info.daoName), PRIVATE).addAnnotation(Inject.class).build());

		// 实体列表首页方法
		builder.addMethod(MethodSpec.methodBuilder("index").addModifiers(PUBLIC).returns(void.class).addParameter(PageModel.class, "model")
				.addAnnotation(AnnotationSpec.builder(Action.class).addMember("url", "$S", "index.htm").build()).addJavadoc("实体列表首页 \n")
				.addJavadoc("@param model 数据模型渲染器 \n").build());

		// 实体列表数据分页方法
		MethodSpec.Builder method = MethodSpec.methodBuilder("pages").addModifiers(PUBLIC).returns(void.class)
				.addParameter(MapModel.class, "model").addParameter(Paging.class, "paging")
				.addAnnotation(AnnotationSpec.builder(Action.class).addMember("value", "$T.class", MapModel.class)
						.addMember("url", "$S", "pages.htm").build())
		...
		method.addStatement("model.addData($S, $N.getTotal())", "count", "paging").addStatement("model.addData($S, $S)", "msg", "获取数据成功")
		...
		//  删除实体信息
		delete(info, pkFieldList, builder);
	...
	private static void insert(ClassInfo info, TypeSpec.Builder builder) {
		builder.addMethod(MethodSpec.methodBuilder("insert").addModifiers(PUBLIC).returns(void.class).addParameter(MapModel.class, "model")
				.addParameter(info.beanClass, firstLowerCase(info.beanName))
				.addAnnotation(AnnotationSpec.builder(Action.class).addMember("value", "$T.class", MapModel.class)
						.addMember("url", "$S", "insert.htm").build())
				.addJavadoc("添加实体处理 \n").addJavadoc("@param model 数据模型渲染器 \n")
				.addJavadoc("@param $N 实体信息 \n", firstLowerCase(info.beanName))
				.addStatement("$N.insert($N)", firstLowerCase(info.daoName), firstLowerCase(info.beanName)).build());
	}

	//  修改实体信息处理
	private static void update(ClassInfo info, TypeSpec.Builder builder) {
		builder.addMethod(MethodSpec.methodBuilder("update").addModifiers(PUBLIC).returns(void.class).addParameter(MapModel.class, "model")
				.addParameter(info.beanClass, firstLowerCase(info.beanName))
				.addAnnotation(AnnotationSpec.builder(Action.class).addMember("value", "$T.class", MapModel.class)
						.addMember("url", "$S", "update.htm").build())
				.addJavadoc("修改实体处理 \n").addJavadoc("@param model 数据模型渲染器 \n")
				.addJavadoc("@param $N 实体信息 \n", firstLowerCase(info.beanName))
				.addStatement("$N.update($N)", firstLowerCase(info.daoName), firstLowerCase(info.beanName)).build());
	}
	...
	private static void delete(ClassInfo info, List<Util.FieldInfo> pkFieldList, TypeSpec.Builder builder) {
		MethodSpec.Builder method = MethodSpec.methodBuilder("delete").addModifiers(PUBLIC).returns(void.class)
				.addParameter(MapModel.class, "model").addAnnotation(AnnotationSpec.builder(Action.class)
						.addMember("value", "$T.class", MapModel.class).addMember("url", "$S", "delete.htm").build())
				.addJavadoc("删除实体信息处理 \n").addJavadoc("@param model 数据模型渲染器 \n");
