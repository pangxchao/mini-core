...
public final class CodeDaoBase {
    ...
    public static void generator(Configure configure, ClassInfo info, BeanItem bean, boolean cover) throws Exception {
        if (!cover && configure.exists(info.daoBasePackage, info.daoBaseName)) {
            return;
        }

        JavaFile.builder(info.daoBasePackage, TypeSpec
                // 接口名称
                .interfaceBuilder(info.daoBaseName)
                // public 接口
                .addModifiers(PUBLIC)
                // 继承 BasicsDao 接口
                ...
                .addMethod(MethodSpecBuilder
                        ...
                        // 方法体， insert inot 语句
                        .addStatement("\tinsert_into($T.TABLE)", info.beanClass)
                        // 为每个字段添加一个键值对 修改语句
                ...
                .addMethod(MethodSpecBuilder
                        ...
                        .addCode("return execute(new $T() {{ \n", SQLBuilder.class)
                        // 方法体 replace info 语句
                        .addStatement("\treplace_into($T.TABLE)", info.beanClass)
                        // 为每个字段添加一个键值修改语句
                        .forAdd(info.fieldList, (method, fieldInfo) -> {
                            // 自动增长字段不处理
                            if (fieldInfo.isAuto()) return;
                ...
                .addMethod(MethodSpecBuilder
                        ...
                        .addCode("return execute(new $T() {{ \n", SQLBuilder.class)
                        // 方法体 updata 语句
                        .addStatement("\tupdate($T.TABLE)", info.beanClass)
                        ...
                        // 为每个主键字段添加一个查询限制条件
                        .forAdd(info.PKFieldList, (method, fieldInfo) -> {
                            String db_name = fieldInfo.getFieldName().toUpperCase();
                            String name = toJavaName(fieldInfo.getFieldName(), true);
                            method.addCode("\t// $L \n", fieldInfo.getRemarks());
                ...
                .addMethod(MethodSpecBuilder
                        ...
                        .addCode("return execute(new $T() {{ \n", SQLBuilder.class)
                        // 方法体 delete 语句
                        .addStatement("\tdelete().from($T.TABLE)", info.beanClass)
                        // 为每个主键信息添加一个查询条件限制
                        .forAdd(info.PKFieldList, (method, fieldInfo) -> {
                            String db_name = fieldInfo.getFieldName().toUpperCase();
                            String name = toJavaName(fieldInfo.getFieldName(), true);
