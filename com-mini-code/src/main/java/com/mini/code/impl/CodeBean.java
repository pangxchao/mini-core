...
public final class CodeBean {
	...
	protected static void run(Configure configure, ClassInfo info, String tableName, String prefix, //
		...
		// 生成类信息
		TypeSpec.Builder builder = TypeSpec.classBuilder(info.beanName).addModifiers(PUBLIC).addSuperinterface(info.baseClass)
				.addSuperinterface(Serializable.class)
				.addField(FieldSpec.builder(long.class, "serialVersionUID").addModifiers(PRIVATE, STATIC, FINAL).initializer("$L", "-1L")
						.build())
				.addField(FieldSpec.builder(String.class, "TABLE").addModifiers(PUBLIC, STATIC, FINAL).addJavadoc(" 表名称 $L \n", tableName)
						.initializer("$S", tableName).build())
				.addJavadoc("$L.java \n", info.beanName).addJavadoc("@author xchao \n");

		// 处理字段常量
		for (Util.FieldInfo fieldInfo : fieldList) {
			String db_name = fieldInfo.getFieldName().toUpperCase();
			builder.addField(FieldSpec.builder(String.class, db_name).addModifiers(PUBLIC, STATIC, FINAL)
					.initializer("$S ", fieldInfo.getColumnName())
					.addJavadoc("$L \n", StringUtil.def(fieldInfo.getRemarks(), fieldInfo.getColumnName())).build());
		}

		// 处理属性
		for (Util.FieldInfo fieldInfo : fieldList) {
			String name = toJavaName(fieldInfo.getFieldName(), false);
			builder.addField(FieldSpec.builder(fieldInfo.getTypeClass(), name).addModifiers(PRIVATE).build());
		}
	...
	private static MethodSpec.Builder getter(Util.FieldInfo fieldInfo, String name) {
		return MethodSpec.methodBuilder("get" + firstUpperCase(name)).addModifiers(PUBLIC)
				// 设置返回类型
				.returns(fieldInfo.getTypeClass()).addAnnotation(Override.class)
				// 方法体内容
	...
	private static MethodSpec.Builder setter(Util.FieldInfo fieldInfo, String name) {
		return MethodSpec.methodBuilder("set" + firstUpperCase(name)).addModifiers(PUBLIC)
				// 设置返回类型
				.returns(void.class)
				// 添加方法参数列表
				.addParameter(fieldInfo.getTypeClass(), name).addAnnotation(Override.class)
				// 添加方法体内容
