...
public abstract class ArgumentResolverBasic implements ArgumentResolver, EventListener {
    private final Map<Class<?>, Function<String, ?>> map = new HashMap<>() {
...

        {
            ...
            put(Long.class, (Function<String, Long>) value -> //
                    Optional.ofNullable(value).filter(v -> !v.isBlank()).map(Long::parseLong).orElse(null));

            // long.class 处理
            put(long.class, (Function<String, Long>) value -> //
                    Optional.ofNullable(value).filter(v -> !v.isBlank()).map(Long::parseLong).orElse(0L));

            // Integer.class 处理
            put(Integer.class, (Function<String, Integer>) value -> //
                    Optional.ofNullable(value).filter(v -> !v.isBlank()).map(Integer::parseInt).orElse(null));

            // int.class 处理
            put(int.class, (Function<String, Integer>) value -> //
                    Optional.ofNullable(value).filter(v -> !v.isBlank()).map(Integer::parseInt).orElse(0));

            // Short.class 处理
            put(Short.class, (Function<String, Short>) value -> //
                    Optional.ofNullable(value).filter(v -> !v.isBlank()).map(Short::parseShort).orElse(null));

            // Short.class 处理
            put(short.class, (Function<String, Short>) value -> //
                    Optional.ofNullable(value).filter(v -> !v.isBlank()).map(Short::parseShort).orElse((short) 0));

            // Byte.class 处理
            put(Byte.class, (Function<String, Byte>) value -> //
                    Optional.ofNullable(value).filter(v -> !v.isBlank()).map(Byte::parseByte).orElse(null));

            // byte.class 处理
            put(byte.class, (Function<String, Byte>) value -> //
                    Optional.ofNullable(value).filter(v -> !v.isBlank()).map(Byte::parseByte).orElse((byte) 0));

            // Double.class 处理
            put(Double.class, (Function<String, Double>) value -> //
                    Optional.ofNullable(value).filter(v -> !v.isBlank()).map(Double::parseDouble).orElse(null));

            // double.class 处理
            put(double.class, (Function<String, Double>) value -> //
                    Optional.ofNullable(value).filter(v -> !v.isBlank()).map(Double::parseDouble).orElse(0D));

            // Float.class 处理
            put(Float.class, (Function<String, Float>) value -> //
                    Optional.ofNullable(value).filter(v -> !v.isBlank()).map(Float::parseFloat).orElse(null));

            // float.class 处理
            put(float.class, (Function<String, Float>) value -> //
                    Optional.ofNullable(value).map(Float::parseFloat).orElse(0F));

            // Boolean.class 处理
            put(Boolean.class, (Function<String, Boolean>) value -> //
                    Optional.ofNullable(value).filter(v -> !v.isBlank()).map(Boolean::parseBoolean).orElse(null));

            // boolean.class 处理
            put(boolean.class, (Function<String, Boolean>) value -> //
                    Optional.ofNullable(value).filter(v -> !v.isBlank()).map(Boolean::parseBoolean).orElse(false));
     ...
     */
    @Inject
    public final void setDateTimeFormat(@Named("mini.http.datetime-format") @Nullable String dateTimeFormat) {
     ...
     */
    @Inject
    public final void setDateFormat(@Named("mini.http.date-format") @Nullable String dateFormat) {
     ...
     */
    @Inject
    public final void setTimeFormat(@Named("mini.http.time-format") @Nullable String timeFormat) {
