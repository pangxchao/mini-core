    ...
    private final Map<Class<?>, Function<String[], ?>> map = new HashMap<>() {
...

        {
            ...
            put(Long[].class, (Function<String[], Long[]>) values -> //
                    Stream.ofNullable(values).flatMap(Stream::of).filter(value -> !value.isBlank()).map(Long::parseLong).toArray(Long[]::new));

            // long[].class 处理
            put(long[].class, (Function<String[], long[]>) values -> //
                    Stream.ofNullable(values).flatMap(Stream::of).filter(value -> !value.isBlank()).mapToLong(Long::parseLong).toArray());

            // Integer[].class 处理
            put(Integer[].class, (Function<String[], Integer[]>) values ->//
                    Stream.ofNullable(values).flatMap(Stream::of).filter(value -> !value.isBlank()).map(Integer::parseInt).toArray(Integer[]::new));

            // int[].class 处理
            put(int[].class, (Function<String[], int[]>) values -> //
                    Stream.ofNullable(values).flatMap(Stream::of).filter(value -> !value.isBlank()).mapToInt(Integer::parseInt).toArray());

            // Short[].class 处理
            put(Short[].class, (Function<String[], Short[]>) values -> //
                    Stream.ofNullable(values).flatMap(Stream::of).map(Short::parseShort).toArray(Short[]::new));

            // Byte.class 处理
            put(Byte[].class, (Function<String[], Byte[]>) values -> //
                    Stream.ofNullable(values).flatMap(Stream::of).filter(value -> !value.isBlank()).map(Byte::parseByte).toArray(Byte[]::new));

            // Double.class 处理
            put(Double.class, (Function<String[], Double[]>) values -> //
                    Stream.ofNullable(values).flatMap(Stream::of).filter(value -> !value.isBlank()).map(Double::parseDouble)
                            .toArray(Double[]::new));

            // double[].class 处理
            put(double[].class, (Function<String[], double[]>) values -> //
                    Stream.ofNullable(values).flatMap(Stream::of).filter(value -> !value.isBlank()).mapToDouble(Double::parseDouble).toArray());
            // Float[].class 处理
            put(Float[].class, (Function<String[], Float[]>) values -> //
                    Stream.ofNullable(values).flatMap(Stream::of).filter(value -> !value.isBlank()).map(Float::parseFloat).toArray(Float[]::new));

            // Boolean[].class 处理
            put(Boolean[].class, (Function<String[], Boolean[]>) values -> //
                    Stream.ofNullable(values).flatMap(Stream::of).filter(value -> !value.isBlank()).map(Boolean::parseBoolean)
                            .toArray(Boolean[]::new));
