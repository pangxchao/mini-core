/**
 * Created the sn.mini.web.SNInitializer.java
...
import sn.mini.web.filter.SNAbstractFilter;
...
import sn.mini.web.http.view.SNView;
import sn.mini.web.listener.SNServletContextAttrListener;
import sn.mini.web.listener.SNServletContextListener;
import sn.mini.web.listener.SNServletRequestAttrListener;
import sn.mini.web.listener.SNServletRequestListener;
import sn.mini.web.listener.SNSessionAttrListener;
import sn.mini.web.listener.SNSessionIdListener;
import sn.mini.web.listener.SNSessionListener;
import sn.mini.web.task.ITask;

/**
 * sn.mini.web.SNInitializer.java
 * @author XChao
 */
@HandlesTypes({ SNConfig.class, Controller.class, Interceptor.class, IRender.class, IView.class,
	SNServletContextListener.class, SNServletContextAttrListener.class, SNSessionListener.class,
	SNSessionAttrListener.class, SNSessionIdListener.class, SNServletRequestListener.class,
	SNServletRequestAttrListener.class, SNAbstractFilter.class })
public final class SNInitializer implements ServletContainerInitializer {
	@SuppressWarnings("unchecked")
	private static final Class<? extends Interceptor>[] INTERS_EMPTY = new Class[0];
		...
		try {
			...
			for (Class<? extends SNAbstractFilter> filter : filters) { // Filter
				...
				if(filter.getAnnotation(WebFilter.class) == null) {
					...
					if(instance.getServletNames() != null && instance.getServletNames().length > 0) {
						dynamic.addMappingForServletNames(instance.getDispatcherType(), true,
							instance.getServletNames());
					}
				}
			}
	...
	protected void controller(List<Class<? extends Controller>> controllers, ServletContext context) {
		for (Class<? extends Controller> controller : controllers) {
			if(controller.getAnnotation(WebServlet.class) != null) {
				throw new RuntimeException("Controller class cannot have WebServlet annotation.");
			}
			...
			try {
				...
				Dynamic register = context.addServlet(instence.getServletName(), instence);// 动态注册Servlet
				for (Method actionMethod : controller.getMethods()) {
					Optional.ofNullable(actionMethod.getAnnotation(Action.class)).ifPresent(action -> {
						...
						ActionProxy proxy = new ActionProxy(instence, control, method, action, name, path, url,
							SNInitializer.getRender(action.value()), view);
