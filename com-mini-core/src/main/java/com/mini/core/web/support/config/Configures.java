...
//	public Configures() {}
	...
	public final Map<Method, ActionSupportProxy> getActionProxy(String requestUri) {
		String uri = Optional.ofNullable(requestUri).map(v -> { // 处理URL开头的 “/”
			return v.startsWith("/") ? v.substring(1) : v; //
		}).orElseThrow();
		// 获取请求映射对象
		return Optional.ofNullable(mappings.get(uri)).orElseGet(() -> mappings.keySet().stream()
				.filter(k -> MATCHER.match(k, uri)).min(MATCHER.getPatternComparator(uri))
