...
public final class JdbcUtil {
    ...
    /**
     * Retrieve a JDBC column value from a ResultSet, using the most appropriate value type. The returned value should be a detached value
     * object, not having any ties to the active ResultSet: in particular, it should not be a Blob or Clob object but rather a byte array or
     * String representation, respectively.
     * <p>
     * Uses the {@code getObject(index)} method, but includes additional "hacks" to get around Oracle 10g returning a non-standard object
     * for its TIMESTAMP datatype and a {@code java.sql.Date} for DATE columns leaving out the time portion: These columns will explicitly
     * be extracted as standard {@code java.sql.Timestamp} object.
     ...
     * <p>
     * Uses the specifically typed ResultSet accessor methods, falling back to {@link #getResultSetValue(java.sql.ResultSet, int)} for
     * unknown types.
     * <p>
     * Note that the returned value may not be assignable to the specified required type, in case of an unknown type. Calling code needs to
     * deal with this case appropriately, e.g. throwing a corresponding exception.
     * @param rs           is the ResultSet holding the data
     * @param index        is the column index
     ...
     * <p>
     * Typically invoked right before execution of a given set of statements: to decide whether the set of SQL statements should be executed
     * through the JDBC 2.0 batch mechanism or simply in a traditional one-by-one fashion.
     * <p>
     * Logs a warning if the "supportsBatchUpdates" methods throws an exception and simply returns {@code false} in that case.
    ...
    public static String commonDatabaseName(@Nullable String source) {
        ...
        if ("Sybase SQL Server".equals(source) || "Adaptive Server Enterprise".equals(source) || "ASE".equals(source)
     ...
     * <p>
     * <i>columnLabel - the label for the column specified with the SQL AS clause. If the SQL AS clause
     * was not specified, then the label is the name of the column</i>.
