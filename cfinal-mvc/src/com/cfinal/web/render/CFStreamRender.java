 ...
 * @author XChao
 */
public class CFStreamRender implements CFRender {

	@Override
	public void render(CFModel model, CFActionPorxy porxy, CFRequest request, CFResponse response) throws Exception {
		try {
			// 设置数据返回的ContentType
			...
			if(model.getContentLength() > 0) {
				...
				if(null != request.getHeader("Range")) {
					...
					response.setStatus(CFResponse.SC_PARTIAL_CONTENT);
